...
const GameCanvas = forwardRef<GameCanvasRef, GameCanvasProps>(
  (
    ...
    const drawIsometricTerrain = (ctx: CanvasRenderingContext2D, camera: { x: number; y: number; zoom: number }) => {
      ...
      renderBurnMarks(ctx, camera)
    }

    const renderBurnMarks = (ctx: CanvasRenderingContext2D, camera: { x: number; y: number; zoom: number }) => {
      const loadedBurnMarks = burnMarkRefs.current.filter((img) => img !== null)
      if (loadedBurnMarks.length === 0) return
      ...
      const visibleLeft = -camera.x - ctx.canvas.width / 2
      const visibleRight = -camera.x + ctx.canvas.width / 2
      const visibleTop = -camera.y - ctx.canvas.height / 2
      const visibleBottom = -camera.y + ctx.canvas.height / 2

      const gridSpacing = 1300

      const startGridX = Math.floor(visibleLeft / gridSpacing) - 1
      const endGridX = Math.ceil(visibleRight / gridSpacing) + 1
      const startGridY = Math.floor(visibleTop / gridSpacing) - 1
      const endGridY = Math.ceil(visibleBottom / gridSpacing) + 1

      ctx.globalCompositeOperation = "multiply"

      for (let gridX = startGridX; gridX <= endGridX; gridX++) {
        for (let gridY = startGridY; gridY <= endGridY; gridY++) {
          const seed = gridX * 73856093 + gridY * 19349663 + currentMap * 83492791

          if (seededRandom(seed) > 0.3) continue

          const offsetRange = 550
          const offsetX = (seededRandom(seed + 4) - 0.5) * offsetRange
          const offsetY = (seededRandom(seed + 5) - 0.5) * offsetRange

          const worldX = gridX * gridSpacing + offsetX
          const worldY = gridY * gridSpacing + offsetY

          const neighborSeed1 = (gridX + 1) * 73856093 + gridY * 19349663 + currentMap * 83492791
          const neighborSeed2 = gridX * 73856093 + (gridY + 1) * 19349663 + currentMap * 83492791
          const neighborIndex1 = Math.floor(seededRandom(neighborSeed1 + 1) * loadedBurnMarks.length)
          const neighborIndex2 = Math.floor(seededRandom(neighborSeed2 + 1) * loadedBurnMarks.length)

          let burnMarkIndex = Math.floor(seededRandom(seed + 1) * loadedBurnMarks.length)

          if (burnMarkIndex === neighborIndex1 || burnMarkIndex === neighborIndex2) {
            burnMarkIndex = (burnMarkIndex + 1) % loadedBurnMarks.length
          }

          const burnMark = loadedBurnMarks[burnMarkIndex]
          if (!burnMark) continue

          const sizeVariation = 0.8 + seededRandom(seed + 2) * 0.6
          const burnSize = 200 * camera.zoom * sizeVariation
          const opacity = 0.8

          const screenX = worldX + camera.x + ctx.canvas.width / 2
          const screenY = worldY + camera.y + ctx.canvas.height / 2

          ctx.save()
          ctx.globalAlpha = opacity
          ctx.translate(screenX, screenY)
          ctx.drawImage(burnMark, -burnSize / 2, -burnSize / 2, burnSize, burnSize)
          ctx.restore()
        }
      }
