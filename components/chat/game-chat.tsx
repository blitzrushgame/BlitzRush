...
interface GameChatProps {
  ...
  username: string
  allianceId?: number
  onCoordinateClick?: (mapId: number, x: number, y: number) => void
}
...
export const GameChat = forwardRef<GameChatRef, GameChatProps>(
  ({ userId, username, allianceId, onCoordinateClick }, ref) => {
    ...
    const renderMessageWithCoordinates = (message: string) => {
      ...
      while ((match = coordPattern.exec(message)) !== null) {
        ...
        parts.push(
          <button
            key={match.index}
            onClick={() => onCoordinateClick?.(mapId, x, y)}
            className="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-600/20 hover:bg-amber-600/40 border border-amber-500/50 rounded text-amber-400 font-semibold transition-colors cursor-pointer"
          >
            Map {mapId} • {x}:{y}
          </button>,
        )
                  ...
                  {globalMessages.map((msg) => (
                    <div key={msg.id} className="flex gap-2 text-xs">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="text-neutral-200 block break-words">
                          {renderMessageWithCoordinates(msg.message)}
                        </span>
                      </div>
                    </div>
                  ))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-neutral-800/50 p-4 rounded-lg border border-amber-500/20">
                      ...
                      <p className="text-xl font-bold text-neutral-200">
                        {selectedUserProfile.points.toLocaleString()}
                      </p>
                    </div>
